...
const getRecordKey = (upload) =>
  String(
    upload?.id ||
      `${upload?.localPath || ''}|${upload?.fileName || ''}|${upload?.uploadedAt || ''}|${upload?.chartDate || ''}`
  )

export default function DailyChartUpload({
  dailyChartUploads,
  setDailyChartUploads,
  currentUser,
  companyId = 'rs_traders'
}) {
  ...
  const sortedUploads = useMemo(
    () =>
      [...dailyChartUploads]
        .filter((u) => (u?.companyId ? u.companyId === companyId : true))
        .sort((a, b) => String(b.chartDate).localeCompare(String(a.chartDate))),
    [dailyChartUploads, companyId]
  )

  const filteredUploads = useMemo(
    () =>
      sortedUploads.filter((u) => {
        if (searchDate && u.chartDate !== searchDate) return false
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      setDailyChartUploads((prev) => [record, ...prev])
      setSelectedFile(null)
      setStatus('Daily chart saved locally.')
    } catch (err) {
  ...
  const removeUpload = async (id) => {
    if (isStaffUser) {
      setStatus('Staff is not allowed to delete daily chart uploads.')
      return
    }
    if (!window.confirm('Delete this upload entry?')) return

    const target = dailyChartUploads.find((u) => getRecordKey(u) === id)
    if (target?.localPath) {
      try {
    ...
    setDailyChartUploads((prev) => prev.filter((u) => getRecordKey(u) !== id))
  ...
  const toggleChecked = (id) => {
    ...
    setDailyChartUploads((prev) =>
      prev.map((u) => (getRecordKey(u) === id ? { ...u, checked: !u.checked } : u))
    )
  }
  ...
  return (
    <section className="page daybook-page">
      <div className="daybook-hero">
        <div>
          <h1>Daily Chart Upload</h1>
          <p>Upload date-wise daily chart files in CSV, XLSX, PDF, PNG, JPG or JPEG format.</p>
        </div>
      ...
      <div className="daybook-form-card">
        ...
        <div className="daybook-actions">
          <button onClick={uploadFile} className="daybook-upload-btn" disabled={uploading}>
            {uploading ? 'Saving...' : 'Upload Daily Chart'}
          </button>
        </div>
            ...
            <tbody>
              {filteredUploads.length ? (
                filteredUploads.map((u) => (
                  <tr key={getRecordKey(u)} className="row-anim">
                    <td>{u.chartDate}</td>
                    <td>{u.fileName}</td>
                    <td>{u.fileType}</td>
                    ...
                    <td>
                      <label className="daybook-check-toggle">
                        <input
                          ...
                          onChange={() => toggleChecked(getRecordKey(u))}
                        />
                    ...
                    <td className="daybook-row-actions">
                      ...
                      <button
                        onClick={() => removeUpload(getRecordKey(u))}
